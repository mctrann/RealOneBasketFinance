...
#12)
select p.firstName,p.lastName, count(o.portfolioID) from Person p
join Owner o on p.personID = o.personID
join Portfolio pf on o.portfolioID = pf.portfolioID
group by p.personID;

#13)
select p.firstName,p.lastName, count(m.portfolioID) from Person p
join Manager m on p.personID = m.personID
join Portfolio pf on m.portfolioID = pf.portfolioID
group by p.personID;
